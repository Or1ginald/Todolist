...
describe("app reducer", () => {
    ...
    it('should delete task', () => {
        ...
        expect(tasksReducer(initialState, action)[ToDoListId1][0].title).toBe("lala");
        expect(tasksReducer(initialState, action)[ToDoListId1][0].title).not.toBe("d");
    })
    it('should update task', () => {
        ...
        const action = updateTaskAC(ToDoListId2, "c4ede16b-272d-4fe6-aac9-41cc949e1145", task)
        expect(tasksReducer(initialState, action)[ToDoListId2].length).toBe(2);
        expect(tasksReducer(initialState, action)[ToDoListId2][0].title).toBe("More sleep");
        expect(tasksReducer(initialState, action)[ToDoListId2][0].title).not.toBe("Eat");
    })
    it('should add task', () => {
        ...
        expect(tasksReducer(initialState, action)[ToDoListId2][0].title).toBe("New Task");
        expect(tasksReducer(initialState, action)[ToDoListId2][0].id).toBe("3");
        expect(tasksReducer(initialState, action)[ToDoListId2]).toStrictEqual(
            [newTask,...initialState[ToDoListId2]]);
    })
    it('should set tasks', () => {
